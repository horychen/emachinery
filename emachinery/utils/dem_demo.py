...
class THE_CONSOLE:
    ...
    def __post_init__(self):
        self.TIME_SLICE = self.d_sim['TIME_SLICE']
        self.NUMBER_OF_TIME_SLICE_TO_SHOW = self.d_sim['NUMBER_OF_SLICES']
        self.END_TIME = self.NUMBER_OF_TIME_SLICE_TO_SHOW*self.TIME_SLICE
        self.MACHINE_SIMULATIONs_PER_SAMPLING_PERIOD = self.d_sim['MACHINE_SIMULATIONs_PER_SAMPLING_PERIOD']
        self.CL_TS = self.d_sim['CL_TS']
        ...
        self.acmsimpy_globals = self.CTRL, self.ACM, self.reg_id, self.reg_iq, self.reg_speed, self.reg_dispX, self.reg_dispY \
            = acmsimpy.Simulation_Benchmark(self.d_sim, tuner=tuner, bool_start_simulation=False).get_global_objects()
...
def run_simulation(CONSOLE):
    ...
    exec(CONSOLE.d_sim['user_system_input_code'])
        ...
        with dpg.collapsing_header(label="Widgets"):
            ...
            with dpg.tree_node(label="Basic"):
                ...
                _help(
                        ...
                        "CTRL+A or REAL-click to select all.\n"
                ...
                dpg.add_input_REAL(label="input REAL", callback=_log, format="%.14f")
                dpg.add_input_REALx(label="input REALx", callback=_log, default_value=[1,2,3,4], format="%.14f")
                dpg.add_drag_int(label="drag int", callback=_log)
                _help(
                        "Click and drag to edit value.\n"
                        "Hold SHIFT/ALT for faster/slower edit.\n"
                        "REAL-click or CTRL+click to input value.")
                ...
                dpg.add_slider_REAL(label="slider REAL", max_value=1.0, format="ratio = %.14f", callback=_log)
            ...
            with dpg.tree_node(label="Multi-component Widgets"):
                ...
                for i in range(2, 5):
                    ...
                    with dpg.group():
                        REAL_source = dpg.add_input_REALx(label=f"input REAL {i}", min_value=0.0, max_value=100.0, size=i)
                        dpg.add_drag_REALx(label=f"drag REAL {i}", source=REAL_source, size=i)
                        dpg.add_slider_REALx(label=f"slider REAL {i}", source=REAL_source, size=i)
            ...
            with dpg.tree_node(label="Vertical Sliders"):
                ...
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_slider_REAL(label=" ", default_value=1.0, vertical=True, max_value=5.0, height=160)
        ...
        with dpg.collapsing_header(label="Plots"):
            ...
            with dpg.tab_bar():
                ...
                with dpg.tab(label="Help"):
                    ...
                    dpg.add_text("REAL left click to fit all visible data.", bullet=True)
                    dpg.add_text("REAL left click on an axis to fit the individual axis", bullet=True, indent=20)
                    dpg.add_text("REAL right click to open the plot context menu.", bullet=True)
        ...
        with dpg.collapsing_header(label="Inputs & Events"):
            ...
            with dpg.tree_node(label="Global"):
                ...
                with dpg.handler_registry(show=False, tag="__demo_mouse_handler"):
                    ...
                    m_REAL_click = dpg.add_mouse_REAL_click_handler(button=dpg.mvMouseButton_Left)
                ...
                mh_REAL = dpg.add_text("Mouse id:", label="Mouse REAL Click Handler", show_label=True)
            ...
            def _event_handler(sender, data):
                ...
                elif type=="mvAppItemType::mvMouseREALClickHandler":
                    dpg.set_value(mh_REAL, f"Mouse id: {data}")
...
def run_dgp_gui():
    ...
    # global data_x, data_y, list_CONSOLE

    # this works even when the module is imported
    data_x = builtins.data_x
    data_y = builtins.data_y
    list_CONSOLE = builtins.list_CONSOLE
    ...
    if False:
        # this calls to realtime_update_data for realtime simulation
        ...
        # this calls to update_plot to do one time plot.
        def _one_time_plot():
            for CONSOLE in list_CONSOLE:
...
if __name__ == '__main__':
    ...
    if True:
        ...
        d['CL_SERIES_KP'] = None
        CONSOLE1 = THE_CONSOLE(name='A', d_sim=copy.deepcopy(d), numba__scope_dict=numba__scope_dict)
        d['FOC_desired_VLBW_HZ'] = 40
        d['CL_SERIES_KP'] = None
        CONSOLE2 = THE_CONSOLE(name='B', d_sim=copy.deepcopy(d), numba__scope_dict=numba__scope_dict)
        d['FOC_desired_VLBW_HZ'] = 20
        d['CL_SERIES_KP'] = None
        CONSOLE3 = THE_CONSOLE(name='C', d_sim=copy.deepcopy(d), numba__scope_dict=numba__scope_dict)
    else:
    ...
    builtins.list_CONSOLE = [CONSOLE1, CONSOLE2, CONSOLE3]
    # Can use collections if you only need the last 100 samples
    builtins.data_x = collections.deque([0.0, 0.0], maxlen=builtins.list_CONSOLE[0].NUMBER_OF_SAMPLE_TO_SHOW)
    builtins.data_y = collections.deque([0.0, 0.0], maxlen=builtins.list_CONSOLE[0].NUMBER_OF_SAMPLE_TO_SHOW)
    run_dgp_gui()
