...
def getBodePlot(CLBW_Hz, currentKp, currentKi, speedKi, speedKp, L, KT, npp, Js, bool_render=False, st=None):
    # Transfer functions
    # current loop zero-pole cancelled already
    ...
    if bool_render:
        fig = plt.figure(101)
    c2c_tf = Gi_closed
    mag, phase, omega = control.bode_plot(c2c_tf, 2*np.pi*np.logspace(
        0, 4, 500), dB=1, Hz=1, deg=1, lw='0.5', label=f'{CLBW_Hz:g=} Hz', plot=bool_render)
    ...
    mag, phase, omega = control.bode_plot(c2v_tf, 2*np.pi*np.logspace(
        0, 4, 500), dB=1, Hz=1, deg=1, lw='0.5', label=f'{CLBW_Hz:g=} Hz', plot=bool_render)
    ...
    mag, phase, omega = control.bode_plot(Gw_closed, 2*np.pi*np.logspace(
        0, 4, 500), dB=1, Hz=1, deg=1, lw='0.5', label=f'Velocity', plot=bool_render)
...
""" legacy codes needed for dem_demo.py """
...
def tunner_wrapper(d_user):
    motor_dict = dict()
    motor_dict['Rs'] =         d_user['init_R']
    motor_dict['Ls'] =         d_user['init_Lq']
    motor_dict['J_s'] =        d_user['init_Js']
    motor_dict['JLoadRatio'] = 0.0
    motor_dict['n_pp'] =       d_user['init_npp']
    motor_dict['KE'] =         d_user['init_KE']
    # motor_dict['KA'] =         d_user['init_KA']
    motor_dict['CL_TS'] =      d_user['CL_TS']
    motor_dict['VL_TS'] =      d_user['CL_TS'] * d_user['VL_EXE_PER_CL_EXE']
    delta = d_user['FOC_delta']
    desired_VLBW_HZ = d_user['FOC_desired_VLBW_HZ']
    ...
    d_user['CL_SERIES_KP'] = currentPI[0]
    d_user['CL_SERIES_KI'] = currentPI[1]
    d_user['VL_SERIES_KP'] = speedPI[0]
    d_user['VL_SERIES_KI'] = speedPI[1]
    d_user['CLBW_HZ'] = BW_in_Hz[0]
    d_user['VLBW_HZ'] = BW_in_Hz[1]
    return d_user


""" END of legacy codes needed for dem_demo.py """


if __name__ == '__main__':
    InstaSPIN_series_PI_tuner(5, 100, 5e-3, 1.1, 0.006,
                              4, 0.09, bool_render=True)
    plt.show()
